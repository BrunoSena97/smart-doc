...
class LLMIntentClassifier:
    ...
    def __init__(self, ollama_url: str = None, model: str = None):
        ...
        sys_logger.log_system(
            "info", f"LLMIntentClassifier initialized with model: {self.model}"
        )
    ...
    def classify_intent(self, doctor_input: str, context: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            sys_logger.log_system(
                "debug",
                f"LLM Intent Classification: '{doctor_input}' -> {result['intent_id']} (confidence: {result['confidence']:.2f})",
            )
    ...
    def classify_intent_with_context(
        self, doctor_input: str, context: str
    ) -> Dict[str, Any]:
        """
        ...
        prompt = self._create_context_aware_prompt(
            doctor_input, context, filtered_intents
        )
        ...
        try:
            ...
            result = self._parse_llm_response_with_context(
                response, doctor_input, valid_intents
            )

            sys_logger.log_system(
                "debug",
                f"Context-Aware LLM Intent Classification [{context}]: '{doctor_input}' -> {result['intent_id']} (confidence: {result['confidence']:.2f})",
            )
            ...
            sys_logger.log_system(
                "warning", f"Context-Aware LLM Intent Classification error: {e}"
            )
            # Fallback to context-aware keyword classification
            return self._fallback_classification_with_context(
                doctor_input, context, valid_intents, str(e)
            )
    ...
    def _create_classification_prompt(self, doctor_input: str) -> str:
        ...
        for intent_id, details in self.intent_categories.items():
            ...
            intent_list.append(
                f"- {intent_id}: {details['description']} (e.g., {examples})"
            )
...
The intent_id MUST be one of the exact IDs listed above. Do not use any other intent names."""
    ...
    def _create_context_aware_prompt(
        self, doctor_input: str, context: str, filtered_intents: Dict[str, Any]
    ) -> str:
        ...
        context_instruction = context_instructions.get(
            context, f"You are in the {context.upper()} phase."
        )
...
The intent_id MUST be one of the exact IDs listed above for the {context} context. Do not use any other intent names."""
    ...
    def _parse_llm_response_with_context(
        self, llm_response: str, original_input: str, valid_intents: set
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if "{" in llm_response and "}" in llm_response:
                ...
                explanation = parsed.get(
                    "explanation", "Context-aware LLM classification"
                )
    ...
    def _fallback_classification_with_context(
        self, doctor_input: str, context: str, valid_intents: set, error_msg: str
    ) -> Dict[str, Any]:
        """
        ...
        if context == "anamnesis":
            ...
            if any(
                ...
                intent_id = (
                    "social_smoking"
                    if "social_smoking" in valid_intents
                    else "clarification"
                )
            ...
            if any(
                ...
                intent_id = (
                    "exam_abdominal"
                    if "exam_abdominal" in valid_intents
                    else "clarification"
                )
            elif any(
                word in input_lower for word in ["neuro", "neurological", "reflexes"]
            ):
                intent_id = (
                    "exam_neurological"
                    if "exam_neurological" in valid_intents
                    else "clarification"
                )
            ...
            if any(
                ...
                intent_id = (
                    "labs_cbc" if "labs_cbc" in valid_intents else "clarification"
                )
            elif any(
                ...
                word in input_lower for word in ["ecg", "ekg", "electrocardiogram"]
            ):
                intent_id = (
                    "imaging_ecg" if "imaging_ecg" in valid_intents else "clarification"
                )
            elif any(
                word in input_lower for word in ["ct scan", "ct", "computed tomography"]
            ):
                intent_id = (
                    "imaging_ct" if "imaging_ct" in valid_intents else "clarification"
                )
            elif any(
                word in input_lower
                for word in ["echo", "echocardiogram", "ultrasound heart"]
            ):
                intent_id = (
                    "imaging_echo"
                    if "imaging_echo" in valid_intents
                    else "clarification"
                )
            elif any(
                word in input_lower for word in ["liver function", "liver enzymes"]
            ):
                intent_id = (
                    "labs_liver" if "labs_liver" in valid_intents else "clarification"
                )

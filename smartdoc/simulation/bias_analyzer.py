...
class BiasEvaluator:
    ...
    def evaluate_session(self, session_log: List[Dict], revealed_blocks: set, 
                        hypotheses: List[Dict], final_diagnosis: str) -> Dict[str, Any]:
        """
        ...
        results = {
            "anchoring_bias": self._detect_anchoring_bias(session_log, hypotheses, final_diagnosis),
            "confirmation_bias": self._detect_confirmation_bias(revealed_blocks),
            "premature_closure": self._detect_premature_closure(revealed_blocks, session_log),
            "overall_score": 0,
            "detailed_analysis": {}
        }
        
        # Calculate overall bias score
        bias_count = sum([1 for bias in results.values() if isinstance(bias, dict) and bias.get("detected", False)])
        results["overall_score"] = bias_count
        
        return results
    
    def check_real_time_bias(self, session_interactions: List[Dict], 
                           current_intent: str, user_input: str, vsp_response: str) -> Dict[str, Any]:
        """
        ...
        if len(session_interactions) < 3:
            return {"detected": False, "reason": "Insufficient interactions for bias detection"}
        
        # Check for different bias types in real-time
        anchoring_result = self._real_time_anchoring_check(session_interactions, current_intent, user_input)
        if anchoring_result["detected"]:
            return anchoring_result
            
        confirmation_result = self._real_time_confirmation_check(session_interactions, current_intent)
        if confirmation_result["detected"]:
            return confirmation_result
            
        premature_result = self._real_time_premature_closure_check(session_interactions, current_intent)
        if premature_result["detected"]:
            return premature_result
            
        return {"detected": False, "reason": "No biases detected in current interaction"}
    
    def _real_time_anchoring_check(self, interactions: List[Dict], current_intent: str, user_input: str) -> Dict[str, Any]:
        ...
        cardio_keywords = ["heart", "cardiac", "chest", "cardiovascular", "failure", "bnp", "echo"]
        ...
        if len(recent_interactions) > 0 and (cardio_count / len(recent_interactions)) > 0.7:
            return {
                "detected": True,
                "bias_type": "anchoring",
    ...
    def _real_time_confirmation_check(self, interactions: List[Dict], current_intent: str) -> Dict[str, Any]:
        """Real-time confirmation bias detection."""
        if not self.bias_triggers.get("confirmation"):
            return {"detected": False, "reason": "No confirmation bias triggers configured"}
        
        # Count confirmatory vs. refuting evidence gathering
        confirmatory_intents = ["lab_tests", "exam_cardiovascular", "vital_signs"]
        broader_intents = ["pmh_general", "social_history", "family_history", "exam_general_appearance"]
        
        recent_interactions = interactions[-7:]  # Larger window for confirmation bias
        
        confirmatory_count = sum(1 for i in recent_interactions 
                               if any(intent in i.get("intent_id", "") for intent in confirmatory_intents))
        broader_count = sum(1 for i in recent_interactions 
    ...
    def _real_time_premature_closure_check(self, interactions: List[Dict], current_intent: str) -> Dict[str, Any]:
        ...
        assessment_count = sum(1 for i in recent_interactions 
                             if any(intent in i.get("intent_id", "") for intent in assessment_intents))
        
        info_count = sum(1 for i in recent_interactions 
                        if any(intent in i.get("intent_id", "") for intent in info_gathering_intents))
    ...
    def _detect_anchoring_bias(self, session_log: List[Dict], hypotheses: List[Dict], 
        ...
        if not hypotheses or not self.bias_triggers.get("anchoring"):
            return {"detected": False, "reason": "No hypotheses or anchoring triggers defined"}
        ...
        if "heart failure" not in initial_hypothesis or "heart failure" not in anchor_info:
            return {"detected": False, "reason": "Initial hypothesis doesn't match expected anchor"}
        ...
        contradictory_revealed = any(
            action.get("action_type") == "view_info" and 
            action.get("details", {}).get("blockId") == contradictory_block_id
        ...
        return {"detected": False, "reason": "Final diagnosis differs from initial anchor"}
    ...
    def _detect_confirmation_bias(self, revealed_blocks: set) -> Dict[str, Any]:
        ...
        if not self.bias_triggers.get("confirmation"):
            return {"detected": False, "reason": "No confirmation bias triggers defined"}
        ...
        support_ratio = len(revealed_supporting) / total_supporting if total_supporting > 0 else 0
        refute_ratio = len(revealed_refuting) / total_refuting if total_refuting > 0 else 0
    ...
    def _detect_premature_closure(self, revealed_blocks: set, session_log: List[Dict]) -> Dict[str, Any]:
        """
        ...
        critical_findings = set(self.case_data.get("groundTruth", {}).get("criticalFindingIds", []))
        ...
        info_gathering_actions = [
            ...
            if action.get("action_type") == "view_info"
        ]
        ...
        min_actions_threshold = 3    # Should perform at least 3 information gathering actions
        
        if (critical_ratio < min_critical_threshold and 
            len(info_gathering_actions) < min_actions_threshold):
            return {
                "detected": True,
    ...
    def generate_feedback_report(self, evaluation_results: Dict[str, Any]) -> str:
        ...
        if bias_count == 0:
            feedback_sections.append("🎉 Excellent! No significant cognitive biases detected in your clinical reasoning.")
        elif bias_count == 1:
            feedback_sections.append("⚠️ One cognitive bias pattern was detected. Review the analysis below to improve your clinical reasoning.")
        else:
            feedback_sections.append(f"🚨 Multiple cognitive bias patterns detected ({bias_count}). This suggests significant room for improvement in clinical reasoning.")
        
        # Specific feedback for each bias
        if evaluation_results["anchoring_bias"]["detected"]:
            feedback_sections.append(self._generate_anchoring_feedback(evaluation_results["anchoring_bias"]))
        
        if evaluation_results["confirmation_bias"]["detected"]:
            feedback_sections.append(self._generate_confirmation_feedback(evaluation_results["confirmation_bias"]))
        
        if evaluation_results["premature_closure"]["detected"]:
            feedback_sections.append(self._generate_closure_feedback(evaluation_results["premature_closure"]))
...
def create_bias_evaluation_demo():
    ...
    case_data = {
        "biasTriggers": {
            ...
            "confirmation": {
                ...
                "refutingInfoIds": ["hist_weight_loss", "pe_cardiac_neg", "critical_formal_cxr", "critical_infliximab", "critical_echo"]
            }
        },
        "groundTruth": {
            "finalDiagnosis": "Miliary tuberculosis",
            "criticalFindingIds": ["hist_weight_loss", "pe_cardiac_neg", "critical_formal_cxr", "critical_infliximab", "critical_echo", "critical_ct_chest"]
        }
    }
    ...
    biased_session = {
        "session_log": [
            {"action_type": "add_hypothesis", "details": {"diagnosis": "Heart failure exacerbation"}, "timestamp": "2025-07-20T10:00:00"},
            {"action_type": "view_info", "details": {"blockId": "pe_resp"}, "timestamp": "2025-07-20T10:01:00"},
            {"action_type": "view_info", "details": {"blockId": "lab_bnp"}, "timestamp": "2025-07-20T10:02:00"},
            {"action_type": "view_info", "details": {"blockId": "critical_echo"}, "timestamp": "2025-07-20T10:03:00"},
            {"action_type": "submit_final_diagnosis", "details": {"diagnosis": "Heart failure exacerbation"}, "timestamp": "2025-07-20T10:04:00"}
        ],

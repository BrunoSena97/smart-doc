...
class SessionLogger:
    ...
    def _check_for_biases(self):
        ...
        recent_interactions = self.session_data["interactions"][-5:]  # Last 5 interactions

        # Check for anchoring bias (focusing too much on heart failure)
        if self._detect_anchoring_bias(recent_interactions):
            self._add_bias_warning("anchoring", "You seem to be focusing heavily on heart failure. Consider other diagnoses.")

        # Check for confirmation bias
        if self._detect_confirmation_bias(recent_interactions):
            self._add_bias_warning("confirmation", "You're seeking information that confirms heart failure. What evidence might contradict it?")

        # Check for premature closure
        if self._detect_premature_closure():
            self._add_bias_warning("premature_closure", "Consider gathering more information before reaching a conclusion.")
    ...
    def _detect_anchoring_bias(self, interactions: List[Dict]) -> bool:
        ...
        cardio_related_intents = [
            "physical_exam_cardiovascular", "vital_signs", "lab_tests",
            "imaging", "ekg", "medications"
        ]

        # Also check for cardiovascular keywords in user queries
        cardio_keywords = [
            "heart", "cardiac", "chest", "cardiovascular", "blood pressure",
            "pulse", "heart rate", "ecg", "ekg", "echo", "bnp"
        ]
        ...
        for interaction in interactions:
            ...
            if any(cardio_intent in intent_id for cardio_intent in cardio_related_intents):
                cardio_count += 1
    ...
    def _detect_confirmation_bias(self, interactions: List[Dict]) -> bool:
        ...
        contradictory_intents = ["social_history", "family_history", "past_medical_history"]
        ...
        for interaction in interactions:
            ...
            if (any(intent in intent_id for intent in confirmatory_intents) or
                any(keyword in user_query for keyword in confirmatory_keywords)):
                confirmatory_count += 1

            # Check contradictory/broader assessment
            if (any(intent in intent_id for intent in contradictory_intents) or
                any(keyword in user_query for keyword in contradictory_keywords)):
    ...
    def _detect_premature_closure(self) -> bool:
        ...
        latest_input = self.session_data["interactions"][-1].get("user_query", "").lower()

        # Keywords that indicate doctor is trying to conclude/diagnose
        conclusion_keywords = [
            "diagnosis", "diagnose", "conclusion", "conclude", "final", "treatment",
            "recommend", "prescription", "discharge", "follow up", "plan",
            "likely", "probably", "most likely", "appears to be", "seems like",
            "let's treat", "start treatment", "prescribe", "medication for",
            "it's", "this is", "you have", "patient has", "have heart failure",
            "start", "begin treatment", "lisinopril", "medication"
        ]
        ...
        has_conclusion_attempt = any(keyword in latest_input for keyword in conclusion_keywords)

        if has_conclusion_attempt:
            # Check how many essential areas have been covered in the entire conversation
            all_inputs = [interaction.get("user_query", "").lower()
                         for interaction in self.session_data["interactions"]]

            covered_areas = 0
            for area, keywords in essential_areas.items():
                if any(keyword in input_text for input_text in all_inputs for keyword in keywords):
                    covered_areas += 1
    ...
    def _add_bias_warning(self, bias_type: str, message: str):
        ...
        recent_biases = [b["bias_type"] for b in self.session_data["detected_biases"][-3:]]
    ...
    def get_session_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "bias_types": list(set([b["bias_type"] for b in self.session_data["detected_biases"]])),
